    ...
    <div className="space-y-4 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Aggregators and DSO market participants</p>
        </div>
        <button
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left text-xs text-gray-500 font-medium px-4 py-3">Market mRID</th>
                  <th className="text-left text-xs text-gray-500 font-medium px-4 py-3">D4G API URL</th>
                  <th className="text-left text-xs text-gray-500 font-medium px-4 py-3">Country</th>
                  <th className="text-right text-xs text-gray-500 font-medium px-4 py-3">Assets</th>
              ...
              <tbody>
                {counterparties.map((cp) => (
                  <tr key={cp.id} className="border-t border-gray-200 hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="text-gray-800 font-medium">{cp.name}</div>
                      {cp.market_role && (
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-gray-500">{cp.market_mrid || '—'}</td>
                    <td className="px-4 py-3">
                      {cp.d4g_api_url ? (
                        <a
                          href={cp.d4g_api_url}
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs">{cp.country || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <button className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 px-2 py-1 rounded border border-gray-200 hover:border-gray-300 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-white border border-gray-200 rounded-xl w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 sticky top-0 bg-white">
              <h2 className="text-sm font-semibold text-gray-900">New Counterparty</h2>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1.5">Market mRID</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1.5">Market Role</label>
                  <select
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">D4G API Base URL</label>
                <input
                  type="url"
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Resource Group ID</label>
                <input
                  type="text"
